"""Local saved tap notes (client-side, user-authored only)."""
...
class SavedTap:
    ...
    def display_subject(self) -> str:
        return (self.subject or self.reminder).strip()
    ...
    def create(
        ...
        if not clean_subject:
            raise ValueError("Tap note subject is required")
        return cls(
            save_id=new_id(),
            peer_id=peer_id,
            peer_name=peer_name,
...
class TapStore:
    ...
    def load(self) -> None:
        ...
        try:
            ...
            for x in raw.get("taps", []):
                legacy = str(x.get("reminder", ""))
                subject = str(x.get("subject", "")).strip() or legacy.strip()
                self._items.append(
                    SavedTap(
    ...
    def get(self, save_id: str) -> SavedTap | None:
        for tap in self._items:
    ...
    def update(self, save_id: str, *, subject: str, detail: str = "") -> bool:
        clean_subject = subject.strip()
        if not clean_subject:
            return False
        for tap in self._items:
            if tap.save_id == save_id:
                tap.subject = clean_subject
                tap.detail = detail.strip()
                tap.reminder = clean_subject
                self.save()
                return True
        return False
